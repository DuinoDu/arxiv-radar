      ...
      <div className="mt-4 divide-y divide-zinc-100 dark:divide-zinc-900">
        {runs.slice(0, 5).map((run) => (
          <Link
            ...
            className="-mx-1 flex flex-wrap items-center justify-between gap-3 rounded-md px-1 py-3 transition hover:bg-zinc-50 first:pt-0 last:pb-0 dark:hover:bg-zinc-900"
            title="查看日志"
          >
            ...
            <span className="text-sm text-zinc-700 dark:text-zinc-300">
              ...
              <span className="ml-2 text-xs text-zinc-400 dark:text-zinc-500">查看日志 →</span>
            </span>
          </Link>
        ))}
      </div>
...
export function PaperDashboard({
  ...
  const handleGithubUrlChange = useCallback((paperId: string, githubUrl: string) => {
    // Optimistically update the paper's githubUrl in local state.
    setPapers((prev) =>
      prev.map((paper) =>
        paper.id === paperId ? { ...paper, githubUrl } : paper,
      ),
    );
    ...
    void fetch(`/api/papers/${encodeURIComponent(paperId)}/github`, {
      ...
      .then(async (response) => {
        ...
        // Update with the server-normalized URL.
        if (payload.githubUrl) {
          setPapers((prev) =>
            prev.map((paper) =>
              paper.id === paperId ? { ...paper, githubUrl: payload.githubUrl } : paper,
            ),
          );
        }
      })

 ...
 * respawns elsewhere to wander again. Pure Canvas 2D, theme-aware, and disabled
...
export function AnimatedGraphBackground() {
  ...
  useEffect(() => {
    ...
    rgbRef.current = isDark ? "161,161,170" : "113,113,122";
  }, [resolvedTheme]);
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      for (const [k, e] of edges) {
        if (dist2(nodes[e.a], nodes[e.b]) > BREAK * BREAK || Math.random() < 0.002) {
          e.dying = true;
        }

#!/usr/bin/env node
// One-off backfill: ask the LLM only "is this paper UMI?" for every visible
// paper that doesn't already have the umi tag, and PATCH the existing
...
//
// Usage:
//   APP_URL=http://localhost:3000 node scripts/backfill-umi-tag.mjs
// Env:
...
const candidates = allPapers.filter((paper) => {
  ...
  return !tags.includes("umi");
});

console.log(
  `state: ${allPapers.length} papers total; ${candidates.length} candidates without umi (skipping removed/already-tagged).`,
);
...
async function applyTag(paper) {
  const nextTags = Array.from(new Set([...(paper.tags || []), "umi"]));
  ...
  while (cursor < candidates.length) {
    ...
    try {
      ...
      if (judgement.umi) {
        ...
        console.log(
          `[worker ${id}] (${index + 1}/${candidates.length}) UMI=true  ${paper.id}  ${paper.title.slice(0, 80)}`,
        );
        ...
        console.log(
          `[worker ${id}] (${index + 1}/${candidates.length}) UMI=false ${paper.id}`,
        );
      }
    } catch (error) {
